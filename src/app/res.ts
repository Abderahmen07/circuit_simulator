...
export class res {
  ...
  constructor(di:any,lin:any,po:any) {
    ...
    this.li=lin;
    this.point=po;
    // Créer l'élément image et charger l'image
    this.img = new Image();
    this.img.onload = () => {
      this.draw();
    };
    this.img.src = 'assets/resistance.jpg';
    // Ajouter les gestionnaires d'événements pour le mouvement de souris
    this.canvas.addEventListener('mousedown', this.handleMouseDown.bind(this));
    this.canvas.addEventListener('mousemove', this.handleMouseMove.bind(this));
    this.canvas.addEventListener('mouseup', this.handleMouseUp.bind(this));
  }
   draw(): void {
    // Dessiner l'image à la position courante
    this.ctx.drawImage(this.img, this.posX, this.posY, 65, 65);
    // Dessiner les cercles de début et de fin
    this.ctx!.beginPath();
  ...
  private handleMouseUp(): void {
    ...
    const point1={x:this.posX + 60, y:this.posY + 32}
    const point2={x:this.posX + 4, y:this.posY + 32}
